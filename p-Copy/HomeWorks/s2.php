...
<body>
    <div class="container">
        ...
        $result = mysqli_query($con, "SELECT studentName,lastHomeWork,alhadithAlakhir,numberSurahs FROM squad2");
        ...
        $idResult = mysqli_query($con, "SELECT id FROM squad2 WHERE studentName = '$StudentName'");
        $row = mysqli_fetch_assoc($idResult);
        $id = $row['id'];


            if (!empty($LastHadeth)) {
                mysqli_query($con, "UPDATE squad2 SET alhadithAlakhir ='$LastHadeth' WHERE id = '$id'");
            }

            if (!empty($LastHomeWork)) {
                mysqli_query($con, "UPDATE squad2 SET lastHomeWork ='$LastHomeWork' WHERE id = '$id'");
            }

            if (!empty($NumberOfSoar)) {
                mysqli_query($con, "UPDATE squad2 SET numberSurahs ='$NumberOfSoar' WHERE id = '$id'");
            }
        ?>
    </div>
</body>
